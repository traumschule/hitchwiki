...
# Based on Parsoid initscript by Gabriel Wicke <gwicke@wikimedia.org>
...
do_start()
{
	...
	# 12.04 LTS, see below
	start-stop-daemon --start --quiet --pidfile $PIDFILE -bm --test \
		--exec /bin/sh -- \
		-c "$DAEMON $DAEMON_ARGS >> /var/log/maildev.log 2>&1" \
		|| return 1
	start-stop-daemon --start --quiet --pidfile $PIDFILE -bm --no-close \
		--exec $DAEMON -- $DAEMON_ARGS >> /var/log/maildev.log 2>&1 \
		|| return 2

	echo "Started Maildev server on port $PORT"
